...
#ifndef BB_MATHVECTOR_H
#define BB_MATHVECTOR_H
...
#define BB_VECTOR_TYPEDEFS(tname, suffix) \
    template <typename T> struct /*BIGFX_API*/ tname; \
...
BB_VECTOR_TYPEDEFS(Vec2, vec2)
BB_VECTOR_TYPEDEFS(Cmplx, cmplx)
BB_VECTOR_TYPEDEFS(Vec3, vec3)
BB_VECTOR_TYPEDEFS(Vec4, vec4)
BB_VECTOR_TYPEDEFS(Quat, quat)
BB_VECTOR_TYPEDEFS(Mat2, mat2)
BB_VECTOR_TYPEDEFS(Mat3, mat3)
BB_VECTOR_TYPEDEFS(Mat4, mat4)

#undef BB_VECTOR_TYPEDEFS
...
template<typename T, int N> struct /*BIGFX_API*/ XVec2
{
...
template<typename T, int N> struct /*BIGFX_API*/ XVec3
{
...
template<typename T, int N> struct /*BIGFX_API*/ XVec4
{
...
#define BB_MEMBER_OPS(tname, first) \
...
template <typename T> struct /*BIGFX_API*/ BVec2
{
...
template <typename T> struct /*BIGFX_API*/ Vec2 : BVec2<T>
{
    inline BB_CONSTEXPR Vec2() {}
    inline BB_CONSTEXPR Vec2(T X, T Y) : BVec2<T>(X, Y) {}

    explicit BB_CONSTEXPR inline Vec2(T X) : BVec2<T>(X, X) {}

    template<int N>
    inline BB_CONSTEXPR Vec2(XVec2<T, N> const &v)
      : BVec2<T>(v[0], v[1]) {}

    template<typename U, int N>
    explicit inline BB_CONSTEXPR Vec2(XVec2<U, N> const &v)
      : BVec2<T>(v[0], v[1]) {}

    BB_MEMBER_OPS(Vec2, x)
...
template <typename T> struct /*BIGFX_API*/ Cmplx
{
    inline BB_CONSTEXPR Cmplx() {}
    inline BB_CONSTEXPR Cmplx(T X) : x(X), y(0) {}
    inline BB_CONSTEXPR Cmplx(T X, T Y) : x(X), y(Y) {}

    BB_MEMBER_OPS(Cmplx, x)
...
template <typename T> struct /*BIGFX_API*/ BVec3
{
...
template <typename T> struct /*BIGFX_API*/ Vec3 : BVec3<T>
{
    inline BB_CONSTEXPR Vec3() {}
    inline BB_CONSTEXPR Vec3(T X, T Y, T Z) : BVec3<T>(X, Y, Z) {}
    inline BB_CONSTEXPR Vec3(Vec2<T> XY, T Z) : BVec3<T>(XY.x, XY.y, Z) {}
    inline BB_CONSTEXPR Vec3(T X, Vec2<T> YZ) : BVec3<T>(X, YZ.x, YZ.y) {}

    explicit inline BB_CONSTEXPR Vec3(T X) : BVec3<T>(X, X, X) {}

    template<int N>
    inline BB_CONSTEXPR Vec3(XVec3<T, N> const &v)
      : BVec3<T>(v[0], v[1], v[2]) {}

    template<typename U, int N>
    explicit inline BB_CONSTEXPR Vec3(XVec3<U, N> const &v)
      : BVec3<T>(v[0], v[1], v[2]) {}
...
template <typename T> struct /*BIGFX_API*/ BVec4
{
...
template <typename T> struct /*BIGFX_API*/ Vec4 : BVec4<T>
{
    inline BB_CONSTEXPR Vec4() {}
    inline BB_CONSTEXPR Vec4(T X, T Y, T Z, T W)
      : BVec4<T>(X, Y, Z, W) {}
    inline BB_CONSTEXPR Vec4(Vec2<T> XY, T Z, T W)
      : BVec4<T>(XY.x, XY.y, Z, W) {}
    inline BB_CONSTEXPR Vec4(T X, Vec2<T> YZ, T W)
      : BVec4<T>(X, YZ.x, YZ.y, W) {}
    inline BB_CONSTEXPR Vec4(T X, T Y, Vec2<T> ZW)
      : BVec4<T>(X, Y, ZW.x, ZW.y) {}
    inline BB_CONSTEXPR Vec4(Vec2<T> XY, Vec2<T> ZW)
      : BVec4<T>(XY.x, XY.y, ZW.x, ZW.y) {}
    inline BB_CONSTEXPR Vec4(Vec3<T> XYZ, T W)
      : BVec4<T>(XYZ.x, XYZ.y, XYZ.z, W) {}
    inline BB_CONSTEXPR Vec4(T X, Vec3<T> YZW)
      : BVec4<T>(X, YZW.x, YZW.y, YZW.z) {}

    explicit inline BB_CONSTEXPR Vec4(T X) : BVec4<T>(X, X, X, X) {}

    template<int N>
    inline BB_CONSTEXPR Vec4(XVec4<T, N> const &v)
      : BVec4<T>(v[0], v[1], v[2], v[3]) {}

    template<typename U, int N>
    explicit inline BB_CONSTEXPR Vec4(XVec4<U, N> const &v)
      : BVec4<T>(v[0], v[1], v[2], v[3]) {}

    BB_MEMBER_OPS(Vec4, x)
...
template <typename T> struct /*BIGFX_API*/ Quat
{
    inline BB_CONSTEXPR Quat() {}
    inline BB_CONSTEXPR Quat(T W) : w(W),  x(0), y(0), z(0) {}
    inline BB_CONSTEXPR Quat(T W, T X, T Y, T Z) : w(W), x(X), y(Y), z(Z) {}

    Quat(Mat3<T> const &m);
    Quat(Mat4<T> const &m);

    BB_MEMBER_OPS(Quat, w)
 ...
 * vec /(vec, vec)
 */
#define BB_VECTOR_VECTOR_OP(tname, op, tprefix, type) \
    tprefix \
 ...
 * vec /=(vec, vec)
 */
#define BB_VECTOR_VECTOR_NONCONST_OP(tname, op, tprefix, type) \
    tprefix \
 ...
 */
#define BB_VECTOR_MINMAX_FUN(tname, op, tprefix, type) \
    tprefix \
 ...
 */
#define BB_VECTOR_CLAMP_FUN(tname, tprefix, type) \
    tprefix \
 ...
 */
#define BB_VECTOR_MIX_FUN(tname, tprefix, type) \
    tprefix \
 ...
 * bool <(vec, vec)
 */
#define BB_VECTOR_VECTOR_BOOL_OP(tname, op, op2, ret, tprefix, type) \
    tprefix \
 ...
 * vec /(vec, scalar)   (also complex & quaternion)
 */
#define BB_VECTOR_SCALAR_OP(tname, op, tprefix, type) \
    tprefix \
 ...
 */
#define BB_SCALAR_VECTOR_OP(tname, op, tprefix, type) \
    tprefix \
 ...
 * vec /=(vec, scalar)   (also complex & quaternion)
 */
#define BB_VECTOR_SCALAR_NONCONST_OP(tname, op, tprefix, type) \
    tprefix \
...
#define BB_UNARY_OPS(tname, tprefix, type) \
    tprefix \
...
#define BB_UNARY_FUNS(tname, tprefix, type) \
    tprefix \
...
#define BB_BINARY_NONVECTOR_FUNS(tname, tprefix, type) \
    tprefix \
...
#define BB_VEC_3_FUNS(tname, tprefix, type) \
    ...
    { \
        return bigfx::abs(a.x) > bigfx::abs(a.z) \
...
#define BB_BINARY_NONVECTOR_OPS(tname, tprefix, type) \
    BB_VECTOR_VECTOR_OP(tname, -, tprefix, type) \
    BB_VECTOR_VECTOR_OP(tname, +, tprefix, type) \
    BB_VECTOR_SCALAR_OP(tname, *, tprefix, type) \
    BB_VECTOR_SCALAR_OP(tname, /, tprefix, type) \
    BB_SCALAR_VECTOR_OP(tname, *, tprefix, type) \
    \
    BB_VECTOR_VECTOR_NONCONST_OP(tname, -, tprefix, type) \
    BB_VECTOR_VECTOR_NONCONST_OP(tname, +, tprefix, type) \
    BB_VECTOR_SCALAR_NONCONST_OP(tname, *, tprefix, type) \
    BB_VECTOR_SCALAR_NONCONST_OP(tname, /, tprefix, type) \
    \
    BB_VECTOR_VECTOR_BOOL_OP(tname, ==, ==, true, tprefix, type) \
    BB_VECTOR_VECTOR_BOOL_OP(tname, !=, ==, false, tprefix, type)

#define BB_BINARY_VECTOR_OPS(tname, tprefix, type) \
    BB_SCALAR_VECTOR_OP(tname, /, tprefix, type)

#define BB_BINARY_VECTOR_FUNS(tname, tprefix, type) \
    BB_VECTOR_MINMAX_FUN(tname, min, tprefix, type) \
    BB_VECTOR_MINMAX_FUN(tname, max, tprefix, type) \
    BB_VECTOR_MINMAX_FUN(tname, fmod, tprefix, type) \
    BB_VECTOR_CLAMP_FUN(tname, tprefix, type) \
    BB_VECTOR_MIX_FUN(tname, tprefix, type) \
    \
    BB_VECTOR_VECTOR_BOOL_OP(tname, <=, <=, true, tprefix, type) \
    BB_VECTOR_VECTOR_BOOL_OP(tname, >=, >=, true, tprefix, type) \
    BB_VECTOR_VECTOR_BOOL_OP(tname, <, <, true, tprefix, type) \
...
#define BB_VECTOR_OPS(tname, tprefix, type) \
    BB_VECTOR_VECTOR_OP(tname, *, tprefix, type) \
    BB_VECTOR_VECTOR_OP(tname, /, tprefix, type) \
    \
    BB_VECTOR_VECTOR_NONCONST_OP(tname, *, tprefix, type) \
...
#define BB_ALL_NONVECTOR_OPS_AND_FUNS(tname) \
    BB_BINARY_NONVECTOR_OPS(tname, template<typename T> static, T) \
    BB_BINARY_NONVECTOR_FUNS(tname, template<typename T> static, T) \
    BB_UNARY_OPS(tname, template<typename T> static, T) \
    BB_UNARY_FUNS(tname, template<typename T> static, T)

#define BB_ALL_VECTOR_OPS_INNER(tname, type) \
    BB_BINARY_VECTOR_OPS(tname, static, type) \
    BB_BINARY_NONVECTOR_OPS(tname, static, type) \
    BB_UNARY_OPS(tname, static, type) \
    BB_VECTOR_OPS(tname, static, type)

#define BB_ALL_VECTOR_FUNS_INNER(tname, type) \
    BB_BINARY_VECTOR_FUNS(tname, static, type) \
    BB_BINARY_NONVECTOR_FUNS(tname, static, type) \
    BB_UNARY_FUNS(tname, static, type)

#define BB_OPEN_NAMESPACE(x) namespace x {
#define BB_CLOSE_NAMESPACE(x) } using namespace x;


/*#define BB_ALL_VECTOR_OPS_AND_FUNS(type) \
    BB_OPEN_NAMESPACE(x##type) \
        BB_ALL_VECTOR_OPS_INNER(Vec2, type) \
		BB_CLOSE_NAMESPACE(x##type) \
	BB_ALL_VECTOR_FUNS_INNER(Vec2, type) */

#define BB_ALL_VECTOR_OPS_AND_FUNS(type) \
    BB_OPEN_NAMESPACE(x##type) \
       BB_ALL_VECTOR_OPS_INNER(Vec2, type) \
       BB_ALL_VECTOR_OPS_INNER(Vec3, type) \
       BB_ALL_VECTOR_OPS_INNER(Vec4, type) \
    BB_CLOSE_NAMESPACE(x##type) \
    BB_ALL_VECTOR_FUNS_INNER(Vec2, type) \
    BB_ALL_VECTOR_FUNS_INNER(Vec3, type) \
    BB_ALL_VECTOR_FUNS_INNER(Vec4, type) \
    BB_VEC_3_FUNS(Vec3, static, type)

BB_ALL_NONVECTOR_OPS_AND_FUNS(Cmplx)
BB_ALL_NONVECTOR_OPS_AND_FUNS(Quat)
...
//BB_ALL_VECTOR_OPS_AND_FUNS(half)
BB_ALL_VECTOR_OPS_AND_FUNS(float)
BB_ALL_VECTOR_OPS_AND_FUNS(double)
//BB_ALL_VECTOR_OPS_AND_FUNS(ldouble)
//BB_ALL_VECTOR_OPS_AND_FUNS(real)

BB_ALL_VECTOR_OPS_AND_FUNS(int8_t)
BB_ALL_VECTOR_OPS_AND_FUNS(uint8_t)
BB_ALL_VECTOR_OPS_AND_FUNS(int16_t)
BB_ALL_VECTOR_OPS_AND_FUNS(uint16_t)
BB_ALL_VECTOR_OPS_AND_FUNS(int32_t)
BB_ALL_VECTOR_OPS_AND_FUNS(uint32_t)
BB_ALL_VECTOR_OPS_AND_FUNS(int64_t)
BB_ALL_VECTOR_OPS_AND_FUNS(uint64_t)
...
#undef BB_VECTOR_VECTOR_OP
#undef BB_VECTOR_VECTOR_NONCONST_OP
#undef BB_VECTOR_MINMAX_FUN
#undef BB_VECTOR_CLAMP_FUN
#undef BB_VECTOR_MIX_FUN
#undef BB_VECTOR_VECTOR_BOOL_OP
#undef BB_VECTOR_SCALAR_OP
#undef BB_SCALAR_VECTOR_OP
#undef BB_VECTOR_SCALAR_OP

#undef BB_BINARY_VECTOR_OPS
#undef BB_BINARY_VECTOR_FUNS
#undef BB_BINARY_NONVECTOR_OPS
#undef BB_BINARY_NONVECTOR_FUNS
#undef BB_UNARY_OPS
#undef BB_UNARY_FUNS
#undef BB_VEC_3_FUNS
#undef BB_VECTOR_OPS

#undef BB_ALL_NONVECTOR_OPS_AND_FUNS
#undef BB_ALL_VECTOR_OPS_INNER
#undef BB_ALL_VECTOR_FUNS_INNER
#undef BB_ALL_VECTOR_OPS_AND_FUNS

#undef BB_OPEN_NAMESPACE
#undef BB_CLOSE_NAMESPACE
...
template <typename T> struct /*BIGFX_API*/ Mat2
{
    inline Mat2() {}
...
template <typename T> struct /*BIGFX_API*/ Mat3
{
    inline Mat3() {}
...
template <typename T> struct /*BIGFX_API*/ Mat4
{
    inline Mat4() {}
...
//BIGBALL_TEMPLATE template struct BIGFX_API Vec3<float>;
